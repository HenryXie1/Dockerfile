...
# Check whether ords is already setup, we will overwrite all configurations later
#if [ ! -f $ORDS_HOME/config/ords/standalone/standalone.properties ]; then
#   setupOrds;
#fi;

# add cp commands to sync configmap for related configfile
cp /mnt/k8s/a*.xml $ORDS_HOME/config/ords/conf/
cp /mnt/k8s/defaults.xml $ORDS_HOME/config/ords/
cp /mnt/k8s/standalone.properties $ORDS_HOME/config/ords/standalone/
