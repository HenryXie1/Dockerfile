...
#Apply db rcu patches
cd $ORACLE_HOME       && \
mv $INSTALL_DIR/$DB_PATCH_1 $ORACLE_HOME/ && \
unzip $DB_PATCH_1 && \
rm $DB_PATCH_1    && \
$ORACLE_HOME/OPatch/opatch apply -silent ./30125133

cd $ORACLE_HOME       && \
mv $INSTALL_DIR/$DB_PATCH_2 $ORACLE_HOME/ && \
mv PatchSearch.xml PatchSearch.xml.old
unzip $DB_PATCH_2 && \
rm $DB_PATCH_2    && \
$ORACLE_HOME/OPatch/opatch apply -silent ./30128191

cd $ORACLE_HOME       && \
mv $INSTALL_DIR/$DB_PATCH_3 $ORACLE_HOME/ && \
unzip $DB_PATCH_3 && \
rm $DB_PATCH_3    && \
$ORACLE_HOME/OPatch/opatch apply -silent ./30083488
